...
module.exports.findUserByEmail = async (email, projection) => {
    const docs = await User.find({ email: email }, projection).limit(1).exec();
    if (docs.length === 0) {
...
module.exports.registerUser = async (name, email, password) => {
    try {
        ...
        console.log(e);
        if (e.code === 11000) throw { status: 409, message: 'User already registered!' };
        throw e;
    }
};
...
module.exports.getProfile = (email) => User.findUserByEmail(email, { name: 1, email: 1, created_at: 1, _id: 0 });
...
module.exports.resetPasswordInit = async (email) => {
    ...
    console.log("Done");
    return { status: 200, message: 'Check mail for instruction' };
};
...
module.exports.resetPasswordFinish = async (email, token, password) => {
    ...
    if (diff >= 2 * 60 * 1000) {
        throw { status: 401, message: 'Time out! Try again' };
    }
    ...
    return { status: 200, message: 'Password changed sucessfully' };
};
